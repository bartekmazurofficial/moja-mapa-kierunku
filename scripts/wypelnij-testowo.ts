 ...
 *
 * Nie zastepuje pilotazu: sluzy do sprawdzenia, czy odpowiedzi trafiaja do
 * bazy w formacie, ktory przyjmuje silnik, i do ogladania raportu w trakcie
 * budowy. Profil ("rzemieslniczy", "spoleczny", "analityczny") steruje tym,
 * ktore pozycje sa wybierane wyzej.
 *
...
async function main() {
  ...
  const profil = (profilArg ?? "rzemieslniczy") as Profil;
  ...
  const ile = await prisma.odpowiedz.count({ where: { uczestnikId: id } });
